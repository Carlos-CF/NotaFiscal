...
public class SolicitacaoDTO {
    ...
    @NotNull(message = "O Numero precisa ser informado!")
    private Long numero;
    
    @NotNull(message = "O Valor precisa ser informado!")
    private Long valor;
    
    @NotNull(message = "Data de Emissão precisa ser informado!")
    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "yyyy-MM-dd")
    @Schema(type = "string", pattern = "^\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}$", example = "2024-01-01")
    private LocalDate dataEmissao;
    
    @NotNull(message = "Data de Vencimento precisa ser informado!")
    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "yyyy-MM-dd")
    @Schema(type = "string", pattern = "^\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}$", example = "2024-01-01")
    private LocalDate dataVencimento;
    
    @NotNull(message = "Data de Entrega precisa ser informado!")
    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "yyyy-MM-dd")
    @Schema(type = "string", pattern = "^\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}$", example = "2024-01-01")
